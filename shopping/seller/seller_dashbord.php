...
// Refactored Session Check:
// Check if the 'seller_id' session variable is NOT set, or if it's empty.
// This indicates the seller is not properly logged in.
if(!isset($_SESSION['seller_id']) || strlen($_SESSION['seller_id']) == 0)
{
    header('location:logout.php'); // Redirect to logout or a generic login page
    exit(); // IMPORTANT: Always use exit() after a header redirect
}
// If the seller IS logged in (i.e., $_SESSION['seller_id'] is set and not empty),
// then the 'else' block from your original code is implicitly where the dashboard content starts.
// We don't need an explicit 'else' if the redirect handles the false case.

// Dashboard Counts (rest of your existing code for seller_dashboard.php)
...
// Code for Registered users (As noted before, this might need adjustment for a seller-specific view)
...
<html lang="en">
    <head>
        ...
        <title>Marketeer | Seller Dashboard</title>
